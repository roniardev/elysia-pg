...
describe("/permission/:id", () => {
    ...
    it("should return unauthorized when no token provided", async () => {
        const response = await fetch(
            `${API_URL}/permission/${TEST_PERMISSION_ID}`,
            {
                method: "GET",
            },
        )
    ...
    it("should return permission not found for invalid permission id", async () => {
        ...
        const response = await fetch(
            `${API_URL}/permission/${invalidPermissionId}`,
            {
                method: "GET",
                headers: {
    ...
    it("should read permission successfully", async () => {
        ...
        const decryptJson = decryptResponse<ReadPermissionResponse["data"]>(
        ...
        expect(decryptJson?.data[0]?.description).toBe(
            "Test permission for reading",
        )
    })
    ...
    afterAll(async () => {
        ...
        await db
            .delete(permissions)
            .where(eq(permissions.id, TEST_PERMISSION_ID))
