...
describe("/permission", () => {
    ...
    beforeAll(async () => {
        ...
        const createPermission = await db.query.permissions.findFirst({
            where: (table, { eq }) =>
                eq(table.name, ManagePermission.CREATE_PERMISSION),
        })
    ...
    it("should create a permission successfully", async () => {
        ...
        const decryptedJson = decryptResponse<CreatePermissionResponse["data"]>(
    ...
    afterAll(async () => {
        ...
        await db
            .delete(permissions)
            .where(eq(permissions.id, createPermissionId))
