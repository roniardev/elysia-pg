...
describe("/permissions", () => {
    ...
    it("should read all permissions successfully", async () => {
        ...
        const decryptJson = decryptResponse<
            ReadAllPermissionsResponse["data"]["data"]
    ...
    it("should filter permissions by search term", async () => {
        ...
        const response = await fetch(
            `${API_URL}/permissions?page=1&limit=10&search=${searchTerm}`,
            {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${accessToken}`,
        ...
        const hasMatchingPermission = decryptJson?.data?.some((permission) =>
            permission.name.includes(searchTerm),
        )
    ...
    afterAll(async () => {
        ...
        await db
            .delete(permissions)
            .where(eq(permissions.id, readAllPermissionId))
