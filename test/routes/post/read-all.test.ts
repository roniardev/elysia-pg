...
describe("/post", () => {
    ...
    it("should return all posts with pagination", async () => {
        ...
        const decryptedJson = decryptResponse<
            ReadAllPostsResponse["data"]["data"]
        >(json.data as unknown as string)
        ...
        if (userPermission) {
            await db
                .delete(scopeUserPermissions)
                .where(
                    eq(
                        scopeUserPermissions.userPermissionId,
                        userPermission.id,
                    ),
                )
        }
    ...
    it("should return posts sorted in ascending order", async () => {
        const response = await fetch(
            `${API_URL}/post?page=1&limit=10&sort=${Sorting.ASC}`,
            {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${accessToken}`,
        ...
        const firstCreatedAt = new Date(decryptedJson?.data[0]?.createdAt ?? 0)
        const lastCreatedAt = new Date(decryptedJson?.data[9]?.createdAt ?? 0)
        expect(firstCreatedAt.getTime() < lastCreatedAt.getTime()).toBe(true)
    })

    it("should return filtered posts by search term", async () => {
        const response = await fetch(
            `${API_URL}/post?page=1&limit=10&search=${twelvePosts[1]?.title}`,
            {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${accessToken}`,
        ...
        expect(decryptedJson?.data[0]?.title).toBe(
            twelvePosts[1]?.title as string,
        )
    })
