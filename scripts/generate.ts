...
import { capitalizeFirstLetter, createDirectoryIfNotExists, generateFile } from './utils/generator';
...
async function generateUsecaseFiles(sourceName: string) {
  ...
  const modelFilePath = join(dataDirPath, `${sourceName.toLowerCase()}.model.ts`);
  ...
  if (existsSync(schemaDirPath)) {
    ...
    const schemaFilePath = join(schemaDirPath, `${sourceName.toLowerCase()}.ts`);
  ...
  // Generate test files
  const testDirPath = join(process.cwd(), 'test', 'routes', sourceName.toLowerCase());
  await createDirectoryIfNotExists(testDirPath);

  const testFiles = [
    { name: 'create.test.ts', template: createTestTemplate(capitalizedSourceName) },
    { name: 'read.test.ts', template: readTestTemplate(capitalizedSourceName) },
    { name: 'read-all.test.ts', template: readAllTestTemplate(capitalizedSourceName) },
    { name: 'update.test.ts', template: updateTestTemplate(capitalizedSourceName) },
    { name: 'delete.test.ts', template: deleteTestTemplate(capitalizedSourceName) }
  ];
  ...
  console.log(`\nNext steps:`);
  console.log(`1. Add ${capitalizedSourceName}Permission to your permissions enum`);
  console.log(`2. Add appropriate success and error messages to your response-message enum`);
  console.log(`3. Update the schema file with any additional fields specific to your model`);
  console.log(`4. Update the data model file with any additional fields specific to your model`);
  console.log(`5. Import and use the schema in your db/schema/index.ts file`);
  console.log(`6. Import and use the ${sourceName.toLowerCase()} module in your server.ts file:`);
  console.log(`   - Add this import: import { ${sourceName.toLowerCase()} } from "@/src/${sourceName.toLowerCase()}";`);
  console.log(`   - Add this line to your app: app.use(${sourceName.toLowerCase()})`);
  console.log(`\nAfter completing these steps, your new ${capitalizedSourceName} endpoints will be available at:`);
  console.log(`- POST /${sourceName.toLowerCase()} - Create a new ${sourceName.toLowerCase()}`);
  console.log(`- GET /${sourceName.toLowerCase()} - Get all ${sourceName.toLowerCase()}s`);
  console.log(`- GET /${sourceName.toLowerCase()}/:id - Get a specific ${sourceName.toLowerCase()}`);
  console.log(`- PUT /${sourceName.toLowerCase()}/:id - Update a specific ${sourceName.toLowerCase()}`);
  console.log(`- DELETE /${sourceName.toLowerCase()}/:id - Delete a specific ${sourceName.toLowerCase()}`);
  console.log(`\nTest files have been generated in test/routes/${sourceName.toLowerCase()}/`);
  console.log(`Run tests with: bun test test/routes/${sourceName.toLowerCase()}`);
}
